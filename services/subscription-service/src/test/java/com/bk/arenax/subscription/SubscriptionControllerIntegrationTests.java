...
class SubscriptionControllerIntegrationTests {
    ...
    void currentSubscriptionReturnsPlanStatusAndEntitlementsForCurrentAccount() throws Exception {
        ...
        mockMvc.perform(trusted(get("/api/v1/subscriptions/current"), accountId))
    ...
    void cancelCurrentSubscriptionMarksItCancelledAndPublishesOutboxEvent() throws Exception {
        ...
        mockMvc.perform(trusted(post("/api/v1/subscriptions/current/cancel"), accountId))
                .andExpect(status().isOk())
    ...
    void currentSubscriptionRejectsMissingAccountContext() throws Exception {
        mockMvc.perform(get("/api/v1/subscriptions/current").header("X-Arenax-User-Id", UUID.randomUUID()))
    ...
    private MockHttpServletRequestBuilder trusted(MockHttpServletRequestBuilder request, UUID accountId) {
        return request.header("X-Arenax-User-Id", UUID.fromString("11111111-1111-1111-1111-111111111111"))
