...
public class MatchCompletedHandler {
  ...
  public void handle(EventEnvelope<MatchCompletedPayload> event) {
    ...
    PlayerRanking winner =
        playerRankingRepository.findByUserId(winnerId).orElseGet(() -> createRanking(winnerId));
    PlayerRanking loser =
        playerRankingRepository.findByUserId(loserId).orElseGet(() -> createRanking(loserId));
    ...
    rankingHistoryRepository.save(
        history(event.payload().matchId(), winnerId, winnerBefore, winnerAfter, "WIN"));
    rankingHistoryRepository.save(
        history(event.payload().matchId(), loserId, loserBefore, loserAfter, "LOSS"));
  }
  ...
  private RankingHistory history(
      UUID matchId, UUID userId, int previousRating, int newRating, String result) {
