...
class MatchCompletedHandlerIntegrationTests {
  ...
  @Autowired private MatchCompletedHandler matchCompletedHandler;
  ...
  void handleUpdatesWinnerAndLoserRankingsAndExposesPlayerQuery() throws Exception {
    ...
    assertThat(playerRankingRepository.findByUserId(winnerId).orElseThrow().getRating())
        .isEqualTo(1016);
    assertThat(playerRankingRepository.findByUserId(loserId).orElseThrow().getRating())
        .isEqualTo(984);
    assertThat(rankingHistoryRepository.findAll()).hasSize(2);

    mockMvc
        .perform(get("/api/v1/rankings/users/{userId}", winnerId))
  ...
  private EventEnvelope<MatchCompletedPayload> rankMatchCompletedEvent(
