...
public class UserRegistrationHandler {
  ...
  public void handle(EventEnvelope<UserRegisteredPayload> event) {
    ...
    Account account =
        accountRepository
            .findByOwnerUserIdAndType(payload.userId(), AccountType.PERSONAL)
  ...
  private void persistOutboxEvent(
      EventEnvelope<UserRegisteredPayload> sourceEvent, Account account) {
    if (outboxEventRepository
        .findByEventTypeAndCorrelationId(
    ...
    EventEnvelope<PersonalAccountCreatedPayload> envelope =
        new EventEnvelope<>(
            ...
            new PersonalAccountCreatedPayload(
                account.getOwnerUserId(), account.getId(), account.getName()));
