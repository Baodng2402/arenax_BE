...
class UserRegisteredEventConsumerIntegrationTests {
  ...
  @Autowired private UserRegisteredEventConsumer consumer;
  ...
  void consumesRegisteredEventCreatesPersonalAccountAndOwnerMembership() throws Exception {
    ...
    Account account =
        accountRepository.findByOwnerUserIdAndType(userId, AccountType.PERSONAL).orElseThrow();
    ...
    assertThat(
            outboxEventRepository.findByEventTypeAndCorrelationId(
                "tenant.personal-account-created.v1", correlationId))
        .isPresent();
  }
