...
public class CompetitionService {
  ...
  private void persistMatchCompleted(Match match) {
    if (outboxEventRepository
        .findByEventTypeAndCorrelationId("competition.match-completed.v1", match.getId())
        .isPresent()) {
      return;
    }

    List<MatchParticipant> participants =
        matchParticipantRepository.findAllByMatchIdOrderByTeamNumberAscCreatedAtAsc(match.getId());
    List<UUID> winners =
        participantIdsForTeam(participants, match.getTeam1Score() >= match.getTeam2Score() ? 1 : 2);
    List<UUID> losers =
        participantIdsForTeam(participants, match.getTeam1Score() >= match.getTeam2Score() ? 2 : 1);

    EventEnvelope<MatchCompletedPayload> envelope =
        new EventEnvelope<>(
            UUID.randomUUID(),
