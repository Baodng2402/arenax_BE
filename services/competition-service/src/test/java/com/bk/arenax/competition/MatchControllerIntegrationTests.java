...
class MatchControllerIntegrationTests {
    ...
    void createJoinAndCompleteRankMatchPublishesMatchCompletedEvent() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/matches/{matchId}/complete", matchId)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {
                                  "team1Score": 3,
        ...
        assertThat(payload.path("payload").path("winners").get(0).asText()).isEqualTo(captainId.toString());
        assertThat(payload.path("payload").path("losers").get(0).asText()).isEqualTo(opponentId.toString());
    ...
    private UUID createSport(String name) throws Exception {
        MvcResult result = mockMvc.perform(post("/api/v1/sports")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {
                                  "name": "%s"
