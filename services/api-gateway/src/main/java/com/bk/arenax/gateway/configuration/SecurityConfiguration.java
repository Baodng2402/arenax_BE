...
public class SecurityConfiguration {
    ...
    JwtDecoder jwtDecoder(GatewayJwtProperties jwtProperties) {
        NimbusJwtDecoder jwtDecoder = NimbusJwtDecoder.withJwkSetUri(jwtProperties.jwkSetUri()).build();
        OAuth2TokenValidator<Jwt> withIssuer = JwtValidators.createDefaultWithIssuer(jwtProperties.issuer());
        OAuth2TokenValidator<Jwt> withAudience = token -> token.getAudience().contains(jwtProperties.audience())
