...
class OutboxEventRelayIntegrationTests {
  ...
  @Test
  void relayPublishesUnpublishedEventsToExchangeAndMarksThemPublished()
    ...
    OutboxEvent alreadyPublished =
        outboxEventRepository.save(
            OutboxEvent.create(
                ...
                "identity-service",
                Instant.now(),
                "{\"eventType\":\"identity.user.password-reset-requested.v1\"}"));
    ...
    verify(rabbitTemplate, times(2))
        .convertAndSend(eq("arenax.events"), routingKey.capture(), message.capture());

    assertThat(routingKey.getAllValues())
    ...
    assertThat(message.getAllValues())
        .containsExactlyInAnyOrder(
            ...
            "{\"eventType\":\"identity.user.verification-requested.v1\"}");
    assertThat(
            outboxEventRepository.findById(alreadyPublished.getId()).orElseThrow().getPublishedAt())
        .isNotNull();
  }
