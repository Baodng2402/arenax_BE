...
class RefreshControllerIntegrationTests {
  ...
  void refreshRotatesRefreshSessionAndReturnsNewAccessToken() throws Exception {
    ...
    List<RefreshSession> refreshSessions =
        refreshSessionRepository.findAll().stream()
  ...
  void refreshPreservesAccountIdInNewAccessToken() throws Exception {
    ...
    assertThat(responseBody.path("user").path("accountId").asText())
        .isEqualTo(accountId.toString());

    List<RefreshSession> refreshSessions = refreshSessionRepository.findAll();
    assertThat(refreshSessions)
        .anySatisfy(session -> assertThat(session.getAccountId()).isEqualTo(accountId));
  }
