...
class UserControllerIntegrationTests {
  ...
  void meReturnsProfileWhenTrustedGatewayHeadersPresent() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/users/me").header("X-Arenax-User-Id", userId.toString()))
  ...
  void meReturnsUnauthorizedForMalformedTrustedUserIdHeader() throws Exception {
    mockMvc
        .perform(get("/api/v1/users/me").header("X-Arenax-User-Id", "not-a-uuid"))
  ...
  void meReturnsNotFoundForUnknownUser() throws Exception {
    mockMvc
        .perform(get("/api/v1/users/me").header("X-Arenax-User-Id", UUID.randomUUID().toString()))
  ...
  void putUsernameSetsPublicHandleAndDeleteClearsIt() throws Exception {
    ...
    mockMvc
        .perform(delete("/api/v1/users/me/username").header("X-Arenax-User-Id", userId.toString()))
  ...
  void putUsernameRejectsUsernameAlreadyTakenByAnotherUser() throws Exception {
    ...
    UUID secondUser =
        UUID.fromString(
            objectMapper
                .readTree(secondRegister.getResponse().getContentAsString())
                .path("userId")
                .asText());
  ...
  void addEmailCreatesSecondaryIdentifierAndSetPrimaryRequiresVerifiedEmail() throws Exception {
    ...
    assertThat(verificationEvent.getEventType())
        .isEqualTo("identity.user.verification-requested.v1");
    assertThat(
            objectMapper
                .readTree(verificationEvent.getPayload())
                .path("payload")
                .path("email")
                .asText())
    ...
    mockMvc
        .perform(
            patch("/api/v1/users/me/emails/{emailId}/primary", emailId)
                .header("X-Arenax-User-Id", userId.toString()))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.code").value("BAD_REQUEST"));
  }
  ...
  @Test
  void verifiedSecondaryEmailCanBecomePrimaryAndOldPrimaryCannotBeDeletedUntilSwitched()
      throws Exception {
    UUID userId = registerAndVerifyUser();
    outboxEventRepository.deleteAll();
    ...
    UserIdentifier originalPrimary =
        userIdentifierRepository.findAll().stream()
    ...
    mockMvc
        .perform(
            delete("/api/v1/users/me/emails/{emailId}", originalPrimary.getId())
                .header("X-Arenax-User-Id", userId.toString()))
        .andExpect(status().isNoContent());
