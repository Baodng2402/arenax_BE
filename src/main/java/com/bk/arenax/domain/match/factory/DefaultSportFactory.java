...
public class DefaultSportFactory implements SportFactory {
  ...
  public Sport createSport(SportCreationContext context) {
    ...
    newSport.setMinPlayersToStart(
        context.minPlayersToStart() != null
            ? context.minPlayersToStart()
            : context.playersPerTeam());
    newSport.setMaxScore(context.maxScore());
    newSport.setScoringType(
        context.scoringType() != null ? context.scoringType().toUpperCase() : "GOALS");
